...
function enqueueUpdate(component) {
  ...
  // 我们代码的各个部分 (例如 ReactCompositeComponent 的 _renderValidatedComponent）假设对render的调用没有嵌套；
  // 验证情况是否如此。（这由每个顶级更新功能（例如setState，forceUpdate等）调用；在ReactMount中保护顶级组件的创建和销毁。）

  // 判断当前是否在更新流程，如果不在更新流程，则调用batchingStrategy.batchedUpdates进行更新
  // batchingStrategy 是 React 进行批处理的一种策略，该策略的实现基于 Transaction，虽然名字和数据库的事务一样，但是做的事情却不一样。
  if (!batchingStrategy.isBatchingUpdates) {
    batchingStrategy.batchedUpdates(enqueueUpdate, component);
    return;
  }
  // 如果在更新流程中，则将待更新组件放入dirtyComponents缓存起来
  // component：组件实例，同reactupdatequeuejs中的internalInstance
