...
function runBatchedUpdates(transaction) {
  ...
  // 对组件调用的优先级进行排序（具体根据shouldComponentUpdateshouldComponentUpdate()），
  // 通过排序，确保在组件的子组件（children）之前调用他们
  dirtyComponents.sort(mountOrderComparator);

  // 在reconciling期间排队的所有更新都必须在整个批次之后执行。
  // 否则，如果脏组件数组（dirtyComponents）是[A, B]，A的children有B和C,
  // B组件将会在一个更新批次中更新两次。
  // 如果C的渲染使B进入更新队列（既然B已经更新了，我们应该跳过它）,而目前唯一
  // 可知的方法就是通过检查批处理计数器
  updateBatchNumber++;
...
// flushBa-tchedUpdates 里面会取出所有的脏组件队列进行 diff，最后更新到 DOM
var flushBatchedUpdates = function () {
  // ReactUpdatesFlushTransaction的包装器将清除dirtyComponents
  // 数组并执行安装就绪处理程序（即componentDidUpdate）排队的所有更新，
  // 但我们也需要在此处进行检查，以捕获由setState回调和asap调用排队的更新。
  while (dirtyComponents.length || asapEnqueued) {
    if (dirtyComponents.length) {
      console.log('flushBatched-Updates...')
...
function enqueueUpdate(component) {
  ...
  if (!batchingStrategy.isBatchingUpdates) {
    ...
    // batchedUpdates具体执行逻辑 ？
    batchingStrategy.batchedUpdates(enqueueUpdate, component);
    return;
  }
  // 如果在更新流程中，则将待更新组件放入dirtyComponents缓存起来，component：组件实例，同 reactupdatequeuejs 中的internalInstance
